@push('title')
    <title>
        {{ ucwords(strtolower($activity->name)) . " | " . ucwords(strtolower($activity->city)). " " . $activity->state }} Martial Art After School</title>
@endpush
...
@section('content')
    ...
    <div id="left-col">
        <a href="/<?php echo $state->statefile?>_youth-sports.html"><?php echo $state->state_name ?> Youth Sports</a> &gt;&gt;
        <?php if ($city):?>
            <a href="/<?php echo $state->statefile?>-youth-sports/<?php echo $city->filename?>.html"><?php echo ucwords(strtolower($city->city)) ?> After School Sports</a> &gt;&gt;
        <?php endif; ?>
            ...
            <div align="center">
                <table width="90%" >
                    ...
                    foreach ($reviews as $review): ?>
                        ...
                        <tr>
                            <td>Comment:</td> <td><?php echo stripslashes($review->comments); ?></td>
                        </tr>
                    <?php endforeach;?>
                </table>
            </div>
        ...
        <?php endif; ?>
        <p> <a name="review"></a>Share your experience with <?php echo ucwords(strtolower($activity->name)) ?>, whether your child attended, you evaluated their services, or you worked there. You can help others by writing a review. </p>
        <br/>
        ...
        <div class="question_section ">
            ...
            <div class="ask-question-btn">
                <input type="button" class="btn" value="Ask a Question" onclick="window.location.href='/send_question?id={{ $activity->id }}&type=youth_sports'" />
            </div>
                ...
                @foreach ($questions as $question)
                    <div class="single-question clinic_table">
                        ...
                        <div class="answer-btn">
                            <input type="button" class="btn" value="Answer the Question Above" onclick="window.location.href='/send_answer?id={{$activity->id}}&type=youth_sports&questionId={{$question->id}}'" />
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div id="right-col">
        {{-- <?php if($activity->claim == '0' || (isset($user->type) && $activity->user_id == $user->id)):?>
            <input type="button" id="edit_btn" name="edit_btn" class="btn" value="Edit" onclick="window.location.href='/activity/edit/id/<?php echo $activity->id ?>'" />
        <?php endif;?> --}}
        <h3>Notes</h3><br/>
